                                     ...
                                     CLHEP::HepRandomEngine* engine)
{
  ...
  float hit_entry_smearedX = hit_entry.x()-fabs(CLHEP::RandGaussQ::shoot(0, 0.1));
  float hit_exit_smearedX = hit_exit.x()+fabs(CLHEP::RandGaussQ::shoot(0, 0.1));
  if (hit_entry.x()>hit_exit.x()) {
    hit_entry_smearedX = hit_entry.x()+fabs(CLHEP::RandGaussQ::shoot(0, 0.1));
    hit_exit_smearedX = hit_exit.x()-fabs(CLHEP::RandGaussQ::shoot(0, 0.1));
  }

  LocalPoint inPoint(hit_entry_smearedX, hit_entry.y(), hit_entry.z());
  LocalPoint outPoint(hit_exit_smearedX, hit_exit.y(), hit_exit.z());
